    ...
    public static char[] decryptTxtFile (char [] arrayInput, int key) {
        ...
        for (int i = 0; i < arrayInput.length; i++) {
            ...
            if (checkRussianSymbolUpper(arrayInput[i])) {
                arrayInput[i] = (char) (((arrayInput[i] + 32 - (Math.abs(key)%32)) - 1040) % 32 + 1040);
            }

            if (checkRussianSymbolLower(arrayInput[i])) {
                arrayInput[i] = (char) (((arrayInput[i] + 32 - (Math.abs(key)%32)) - 1072) % 32 + 1072);
            }

            if (checkSignSymbol(arrayInput[i])) {
                arrayInput[i] = (char) (((arrayInput[i] + 32 - (Math.abs(key)%32)) - 32) % 32 + 32);
            }
        }
        return arrayInput;
    }
